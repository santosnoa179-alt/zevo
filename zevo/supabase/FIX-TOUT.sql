-- ============================================================
-- FIX TOUT EN UN — Copier-coller ce script EN ENTIER
-- ============================================================
...
-- 2. Recréer is_admin() SANS cascade (pas de drop)
...
-- 3. Drop toutes les policies (IF EXISTS = safe)
...
create policy "habitudes_select" on habitudes
  for select using (
    auth.uid() = client_id or auth.uid() = assigned_by
