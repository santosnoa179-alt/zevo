...
export default function MessagesClientPage() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col h-[100dvh] w-full items-center justify-center bg-[var(--bg-base)]">
  ...
  if (!coachId) {
    return (
      <div className="flex flex-col h-[100dvh] w-full items-center justify-center p-4 text-center bg-[var(--bg-base)]">
        ...
        <p className="text-[var(--text-muted)] text-sm font-medium">Aucun coach associé à votre compte</p>
        <p className="text-[var(--text-muted)] text-xs mt-1">Contactez l'administration pour être rattaché</p>
      </div>
    )
  }

  return (
    <div className="flex flex-col h-[100dvh] w-full bg-[var(--bg-base)]">

      {/* ═══════════════ HEADER ═══════════════ */}
      <div className="shrink-0 px-4 py-3 border-b border-[var(--border-base)] bg-[var(--bg-base)]/95 backdrop-blur-xl">
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            {/* Online indicator */}
            <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-emerald-400 border-2 border-[var(--bg-base)]" />
          </div>

          <div className="flex-1 min-w-0">
            <h1 className="text-[var(--text-primary)] font-semibold text-[15px] truncate">{coachName}</h1>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 relative">
        {messages.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-[var(--text-primary)] font-semibold text-base mb-1">Démarre la conversation</p>
            <p className="text-[var(--text-muted)] text-sm">Envoie un message à {coachName}</p>
          </div>
        ) : (
          <>
      ...
      <form
        ...
        className="shrink-0 flex items-end gap-2 px-3 py-2.5 bg-[var(--bg-base)] border-t border-[var(--border-base)]"
